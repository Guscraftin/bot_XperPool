...
const { channel_all_missions, color_accept } = require(process.env.CONST);
...
Bien cordialement,`).setColor(color_accept);
        await channelDetail.send({ content: `||<@${logMission.user_id}>||`, embeds: [embedAnswer] })

        // Modify the initial message
        const embed = new EmbedBuilder(interaction.message.embeds[0]);
        embed.setColor(color_accept);
        await interaction.message.edit({ embeds: [embed], components: [] });

        return interaction.reply({ content: "La candidature a bien été acceptée.", ephemeral: true });
    },
};
